...
test.describe("ArticlePreview component", () => {
  ...
  test("should render article title", async ({ page }) => {
    ...
    if (count > 0) {
      // Filter out the loading/empty state divs and only check actual article previews
      const articlesWithTitle = page.locator(".article-preview h1");
      const titleCount = await articlesWithTitle.count();

      if (titleCount > 0) {
        const firstTitle = articlesWithTitle.first();
  ...
  test("should render article description", async ({ page }) => {
    ...
    if (count > 0) {
      // Look for articles that have the preview-link structure (actual articles, not loading/empty state)
      const articleLinks = page.locator(".article-preview .preview-link");
      const linkCount = await articleLinks.count();

      if (linkCount > 0) {
        const firstLink = articleLinks.first();
        const description = firstLink.locator("p");
  ...
  test('should render "Read more..." link', async ({ page }) => {
    ...
    if (count > 0) {
      // Look for articles with preview-link (actual articles, not loading/empty state)
      const readMoreLinks = page.locator(
        '.article-preview .preview-link:has-text("Read more...")'
      );
  ...
  test("should have link to article detail page", async ({ page }) => {
    ...
    if (count > 0) {
      const firstArticle = articles.first();
      const previewLink = firstArticle.locator(
        'a.preview-link, a[href*="/article/"]'
      );
  ...
  test.describe("favorite button", () => {
    test("should show correct button style based on favorited state", async ({
      page,
    }) => {
      const articles = page.locator(".article-preview");
      const count = await articles.count();

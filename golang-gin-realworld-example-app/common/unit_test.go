...
func TestConnectingDatabase(t *testing.T) {
	...
	// Test DB exceptions - suppress expected error output
	os.Chmod("./../gorm.db", 0000)
	// Redirect stdout to suppress expected error message
	oldStdout := os.Stdout
	os.Stdout = nil
	db = Init()
	os.Stdout = oldStdout
...
func TestConnectingTestDatabase(t *testing.T) {
	...
	// Test create & close DB
	db := TestDBInit()
	db.LogMode(false) // Disable log mode for cleaner output
	_, err := os.Stat("./../gorm_test.db")
	asserts.NoError(err, "Db should exist")
	asserts.NoError(db.DB().Ping(), "Db should be able to ping")
	db.Close()

	// Test testDB exceptions - suppress expected error output
